...
folder_info = []
load_dotenv()
api_key = os.getenv("DH_API")
username = os.getenv("DH_USER")
project_id = os.getenv("DH_PRJ")
base_url = os.getenv("SPIRA_URL_DH")
headers = json.loads(os.getenv("STD_HEADERS_DH", "{}"))
...
def create_payload(df):
    ...
    for index, row in df.iterrows():
        ...
        if row_type.lower() == "folder":
            ...
            if folder_response.status_code in [400]:
                try:
                    res = folder_response.json()
                    print(f"Error 400: {res}")
                except json.JSONDecodeError:
                    print(f"Error 400: {folder_response.text}")
                ...
                if pd.notna(row["Parent Folder"]):
                    for folder in folder_info:
                        if folder["Name"].strip().lower() == str(row["Parent Folder"]).strip().lower():
                            ...
                            move_folder = requests.put(f"{base_url}/projects/{project_id}/test-folders",
                                                       headers=headers, json=folder_move_payload)
                        else:
                            continue
                else:
                    continue
            ...
            response = requests.post(f"{base_url}/projects/{project_id}/test-cases", json=create_test_payload,
                                     headers=headers)
            ...
            if response.status_code in [200, 201, 202]:
                ...
                folder_move = requests.post(
                    f"{base_url}/projects/{project_id}/test-cases/{case_id}/move?test_case_folder_id={folder_id}",
                    headers=headers)
            ...
            test_step_response = requests.post(f"{base_url}/projects/{project_id}/test-cases/{case_id}/test-steps",
...
export_path = "testcaseoutput1.csv"
